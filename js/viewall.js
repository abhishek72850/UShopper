$(document).ready(function(){
	...
	$('.load-shop').on('click',function(){
		...
		var json={
			'offset':uShopper.offset+1,
			'param':'0,0',
			'view':uShopper.search
		}

		var request=$.ajax({
				url:"myvendor/viewmanager.php",
				method:"POST",
				dataType:"text",
				data:json,
			});
	...
	var AsyncFilterSearch=function(detach,opts={}){
		...
		request.done(function(data){ 
			...
			if(data.success){
				...
				var stateObj={source:"viewall.php"};
				//console.log(uShopper.sort+uShopper.filter);
				if(uShopper.sort!="0"&&getFilterType()!="0"){
					history.pushState(stateObj,"viewall","viewall.php?view="+uShopper.search+"&sort="+uShopper.sort+"&filter="+getFilterType()+"&pricemin="+uShopper.pricemin+"&pricemax="+uShopper.pricemax+"&discount="+uShopper.disValue+"&prate="+uShopper.prateValue+"&srate="+uShopper.srateValue);
				}
				else if(getFilterType()!="0"){
					history.pushState(stateObj,"viewall","viewall.php?view="+uShopper.search+"&filter="+getFilterType()+"&pricemin="+uShopper.pricemin+"&pricemax="+uShopper.pricemax+"&discount="+uShopper.disValue+"&prate="+uShopper.prateValue+"&srate="+uShopper.srateValue);	
				}
				else if(uShopper.sort!="0"){
					history.pushState(stateObj,"viewall","viewall.php?view="+uShopper.search+"&sort="+uShopper.sort);	
				}
