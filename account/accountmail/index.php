...
<head>
	...
	<style type="text/css">
		.account-navigation>div:nth-child(4)>ul>li:nth-child(5)>a{
	...
	<main>
		...
		<section>
			...
			<div class="account-category">
				<div class="cat-head">
					<h3>UPDATE EMAIL AND MOBILE</h3>
				</div>
				<div class="cat-body">
					<div>
						<div>
							<span class="input input--nao">
								<input class="input__field input__field--nao" disabled type="email" id="addemail" name="addemail" value="<?php echo $user->detail()['uemail'] ?>" />
								<label class="input__label input__label--nao" for="addemail">
							...
							<span class="email-edit">Edit</span>
							<span class="email-add-cancel" style="display: none;">Cancel</span>
						</div>
						<div>
							<span class="input input--nao">
								<input class="input__field input__field--nao" type="text" id="emailvcode" name="emailvcode" />
								<label class="input__label input__label--nao" for="emailvcode">
									<span class="input__label-content input__label-content--nao">Enter Verification Code</span>
								</label>
								<svg class="graphic graphic--nao" width="300%" height="100%" viewBox="0 0 1200 60" preserveAspectRatio="none">
									<path d="M0,56.5c0,0,298.666,0,399.333,0C448.336,56.5,513.994,46,597,46c77.327,0,135,10.5,200.999,10.5c95.996,0,402.001,0,402.001,0"/>
								</svg>
							</span>
							<span class="email-code-resend">Resend</span>
						</div>
						<div>
							<span class="input input--nao">
								<input class="input__field input__field--nao" disabled type="phone" id="changephone" name="changephone" value="<?php echo $user->detail()['umobile'] ?>" />
								<label class="input__label input__label--nao" for="changephone">
							...
							<span class="phone-edit">Edit</span>
							<span class="phone-add-cancel" style="display: none;">Cancel</span>
						</div>
						<div>
							<span class="input input--nao">
								<input class="input__field input__field--nao" type="text" id="phonevcode" name="phonevcode" />
								<label class="input__label input__label--nao" for="phonevcode">
									<span class="input__label-content input__label-content--nao">Enter Verification Code</span>
								</label>
								<svg class="graphic graphic--nao" width="300%" height="100%" viewBox="0 0 1200 60" preserveAspectRatio="none">
									<path d="M0,56.5c0,0,298.666,0,399.333,0C448.336,56.5,513.994,46,597,46c77.327,0,135,10.5,200.999,10.5c95.996,0,402.001,0,402.001,0"/>
							...
							<span class="phone-code-resend">Resend</span>
						</div>
						<div>
							<input type="submit" value="Save Changes" name="savechange"/>
						</div>
					</div>
				</div>
			</div>
  	...
  	<script>
  		...
  		$(document).ready(function(){
  			...
  			$('.email-edit').on('click',function(){
  				...
  				$(this).addClass('e-go');
  				$(this).removeClass('email-edit');
  				$(this).text('Send Code');
  				$('.email-add-cancel').show();
  			});
  			...
  			$('.phone-edit').on('click',function(){
  				...
  				$(this).text('Send Code');
  				$('.phone-add-cancel').show();
  			});

  			$('.email-add-cancel').on('click',function(){
  				$('#addemail').prop('disabled',true);
  				$('.e-go').addClass('email-edit');
  				$('.email-edit').removeClass('e-go');
  				$('.email-edit').text('Edit');
  				$(this).hide();
  			});

  			$('.phone-add-cancel').on('click',function(){
  				$('#changephone').prop('disabled',true);
  				$('.p-go').addClass('phone-edit');
  				$('.phone-edit').removeClass('p-go');
  				$('.phone-edit').text('Edit');
  				$(this).hide();
  			});

  			$('.e-go').on('click',function(){
  				
  			});

  			$('.p-go').on('click',function(){
