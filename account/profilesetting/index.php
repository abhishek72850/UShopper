<?php
	...
	if (isset($_SESSION['id'])&&isset($_SESSION['email'])&&isset($_SESSION['name'])) {
		...
		if(isset($_FILES['dp'])){
			...
			if($size>3||(!preg_match("/^image\//", $image['type']))){

			}
			else{
				$ext=explode('.', $image['name'])[1];
				$filename="images/".$_SESSION['id'].".".$ext;
				UserManager::updateProfilePic($db,$image['tmp_name'],$filename,$_SESSION['id']);
			}
		}
	}
...
<head>
	...
	<style type="text/css">
		.account-navigation>div:nth-child(4)>ul>li:nth-child(4)>a{
	...
	<main>
		...
		<section>
			...
			<div class="account-category">
				...
				<div class="cat-body">
					<div>
						<p>Change your profile picture from here by uploading image from your local computer.</p>
						
						<div>
							<img src="<?php echo $upic ?>" id="dp-preview">
							<div>
								<div>
									<form action="<?php echo $_SERVER['REQUEST_URI'] ?>" method='post' enctype="multipart/form-data">
										<label for="browse">Browse</label>
										<input type="file" accept="image/*" name="dp" id="browse">
										<input type="submit" name="upload" value="Upload Photo" disabled>
									</form>
								</div>
								<span id="dp-path">Path of the file</span>
							</div>
						</div>
					</div>
				</div>
  ...
  <script type="text/javascript">
  	...
  	$(document).ready(function(){
  		...
  		$('#browse').on('change',function(){
  			...
  			var exp=/^image\//;
  			if(size>3){
  				createError('File size exceeded the size');
  				exit(0);
  			}
  			if(!exp.test(this.files[0].type)){
  				createError('Selected file is not valid image file');
  				exit(0);	
  			}

  			var image=window.URL.createObjectURL(this.files[0]);

  			$('#dp-path').text(this.files[0].name);

  			$('#dp-preview').attr('src',image);
  			$('#dp-preview').onload=function(){
