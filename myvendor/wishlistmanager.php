<?php
	...
	class WishlistManager
	{
		...
		public function checkItem($pid){
			$result=$this->queryParse("SELECT * FROM wishlist_table WHERE uid='".$this->uid."' AND id='".$pid."' AND type='product'");

			return $result["progress"];
		}

		public static function checkInWishlist($db,$pid,$uid,$type){
			
			$sql="SELECT * FROM wishlist_table WHERE uid='".$uid."' AND id='".$pid."' AND type='".$type."'";

			$result=mysqli_query($db,$sql);

			if(mysqli_num_rows($result)>0){
				
				return '<img class="fav_ico" src="images/fav_yes.svg" data-item-id="'.$pid.'" data-item-inWishlist="1" data-item-type="'.$type.'" data-task="wishlist_remove" data-task-command="delete">';
			}
			else{
				return '<img class="fav_ico" src="images/fav_no.svg" data-item-id="'.$pid.'" data-item-inWishlist="0" data-item-type="'.$type.'" data-task="wishlist_add" data-task-command="add">';
			}
		}		

		public function removeFromList($pid){
			$sql="DELETE FROM wishlist_table WHERE uid='".$this->uid."' AND id='".$pid."' AND type='product'";
		...
		public function addItem($pid){
			$sql="INSERT INTO wishlist_table (uid,id,type,date,time) VALUES('".$this->uid."','".$pid."','product',NOW(),NOW())";
		...
		public function getList($type){
			...
			if(mysqli_num_rows($result)>0){
				...
				return json_encode(array("success"=>true,"error"=>mysqli_error($this->db),"list"=>$parray,"length"=>count($parray)));
			}
			else{
	...
	if(isset($_POST['id'])&&isset($_POST['email'])&&isset($_POST['data'])){
		...
		if($wish->verifyUser()){
			...
			else if($_POST['data']==='add' && isset($_POST['pid'])){
				
				$wish->addToWishlist($_POST['pid']);
			}
